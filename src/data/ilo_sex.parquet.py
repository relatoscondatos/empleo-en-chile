...
filtered_emp_df = emp_df[(emp_df['sector'] == 'Total')]

# Pivot the DataFrame to get 'sector' values as columns
pivoted_emp_df = filtered_emp_df.pivot_table(index=['country', 'countryCode', 'time'], columns='sex', values='value').reset_index()

# Calculate the publicPercentage
pivoted_emp_df['femalePercentage'] = pivoted_emp_df['Female'] / pivoted_emp_df['Total']
