...
# Perform the aggregations grouped by 'actividad'
result = data.groupby(['año', 'mes', 'actividad']).agg(
...
# Ensure the data is sorted by year, month, and actividad
result = result.sort_values(by=['mes', 'año', 'actividad'])

# Calculate the year-over-year difference for each field within each actividad
fields_to_diff = [
...
for field in fields_to_diff:
    result[f'{field}_diff'] = result.groupby(['actividad', 'mes'])[field].diff()
